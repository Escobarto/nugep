...
for module in ['streamlit', 'pandas', 'folium', 'streamlit_folium', 'geopy', 'PIL', 'requests', 'plotly', 'openpyxl']:
    try:
        imported_module = __import__(module)
        st.write(f"{module} version: {imported_module.__version__}")
    except ImportError as e:
        st.error(f"Error importing {module}: {e}")
    except AttributeError:
        st.warning(f"{module} imported, but __version__ not available")
